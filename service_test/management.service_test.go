...
func TestCreateStudentByClassIDAndCreateUser(t *testing.T) {
	serv, err := CreateTestManagementService()
	assert.Nil(t, err)
	...
	student2 := entity.ToAddStudent{
		StudentID: "20221050001",
		Name:      "ly",
		Gender:    1,
	}

	student3 := entity.ToAddStudent{
		StudentID: "20221050002",
		Name:      "lyky",
		Gender:    0,
	}
	...
	err = serv.CreateStudentByClassIDAndCreateUser(students, 3)
	assert.Nil(t, err)
...
func TestFindClassesByMajorID(t *testing.T) {
	serv, err := CreateTestManagementService()
	assert.Nil(t, err)

	cs, err := serv.FindClassesByMajorID(uint16(6))
	assert.Nil(t, err)
	for _, v := range cs {
		fmt.Println(v)
	}
