...
func (pq *ProjectQuery) QueryAttachments() *FileQuery {
...
func (pq *ProjectQuery) WithAttachments(opts ...func(*FileQuery)) *ProjectQuery {
	query := (&FileClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withUser != nil {
			_spec.Node.AddColumnOnce(project.FieldUID)
		}
	}
