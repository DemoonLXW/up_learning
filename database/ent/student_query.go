...
func (sq *StudentQuery) QuerySchool() *SchoolQuery {
	query := (&SchoolClient{config: sq.config}).Query()
...
func (sq *StudentQuery) FirstID(ctx context.Context) (id uint32, err error) {
	var ids []uint32
...
func (sq *StudentQuery) FirstIDX(ctx context.Context) uint32 {
...
func (sq *StudentQuery) OnlyID(ctx context.Context) (id uint32, err error) {
	var ids []uint32
...
func (sq *StudentQuery) OnlyIDX(ctx context.Context) uint32 {
...
func (sq *StudentQuery) IDs(ctx context.Context) (ids []uint32, err error) {
...
func (sq *StudentQuery) IDsX(ctx context.Context) []uint32 {
	ids, err := sq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (sq *StudentQuery) WithSchool(opts ...func(*SchoolQuery)) *StudentQuery {
	query := (&SchoolClient{config: sq.config}).Query()
...
func (sq *StudentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Student, error) {
	...
	if query := sq.withSchool; query != nil {
		if err := sq.loadSchool(ctx, query, nodes, nil,
			func(n *Student, e *School) { n.Edges.School = e }); err != nil {
			return nil, err
		}
	}
...
func (sq *StudentQuery) loadSchool(ctx context.Context, query *SchoolQuery, nodes []*Student, init func(*Student), assign func(*Student, *School)) error {
	ids := make([]uint16, 0, len(nodes))
	nodeids := make(map[uint16][]*Student)
	for i := range nodes {
		fk := nodes[i].Sid
...
func (sq *StudentQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Student, init func(*Student), assign func(*Student, *User)) error {
	...
	nodeids := make(map[uint32][]*Student)
	for i := range nodes {
		fk := nodes[i].UID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
...
func (sq *StudentQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(student.Table, student.Columns, sqlgraph.NewFieldSpec(student.FieldID, field.TypeUint32))
	_spec.From = sq.sql
	...
	if fields := sq.ctx.Fields; len(fields) > 0 {
		...
		if sq.withSchool != nil {
			_spec.Node.AddColumnOnce(student.FieldSid)
		}
		if sq.withUser != nil {
			_spec.Node.AddColumnOnce(student.FieldUID)
		}
	}
