...
func (cq *ClassQuery) QueryCollege() *CollegeQuery {
	query := (&CollegeClient{config: cq.config}).Query()
...
func (cq *ClassQuery) FirstID(ctx context.Context) (id uint32, err error) {
	var ids []uint32
...
func (cq *ClassQuery) FirstIDX(ctx context.Context) uint32 {
...
func (cq *ClassQuery) OnlyID(ctx context.Context) (id uint32, err error) {
	var ids []uint32
...
func (cq *ClassQuery) OnlyIDX(ctx context.Context) uint32 {
...
func (cq *ClassQuery) IDs(ctx context.Context) (ids []uint32, err error) {
...
func (cq *ClassQuery) IDsX(ctx context.Context) []uint32 {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *ClassQuery) Clone() *ClassQuery {
	...
	return &ClassQuery{
		...
		withCollege:  cq.withCollege.Clone(),
...
func (cq *ClassQuery) WithCollege(opts ...func(*CollegeQuery)) *ClassQuery {
	query := (&CollegeClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCollege = query
	return cq
}
...
func (cq *ClassQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Class, error) {
	...
	if query := cq.withCollege; query != nil {
		if err := cq.loadCollege(ctx, query, nodes, nil,
			func(n *Class, e *College) { n.Edges.College = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *ClassQuery) loadCollege(ctx context.Context, query *CollegeQuery, nodes []*Class, init func(*Class), assign func(*Class, *College)) error {
	ids := make([]uint16, 0, len(nodes))
	nodeids := make(map[uint16][]*Class)
	for i := range nodes {
		fk := nodes[i].Cid
...
func (cq *ClassQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(class.Table, class.Columns, sqlgraph.NewFieldSpec(class.FieldID, field.TypeUint32))
	_spec.From = cq.sql
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withCollege != nil {
			_spec.Node.AddColumnOnce(class.FieldCid)
		}
	}
