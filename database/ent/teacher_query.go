...
func (tq *TeacherQuery) QueryCollege() *CollegeQuery {
	query := (&CollegeClient{config: tq.config}).Query()
...
func (tq *TeacherQuery) FirstID(ctx context.Context) (id uint32, err error) {
	var ids []uint32
...
func (tq *TeacherQuery) FirstIDX(ctx context.Context) uint32 {
...
func (tq *TeacherQuery) OnlyID(ctx context.Context) (id uint32, err error) {
	var ids []uint32
...
func (tq *TeacherQuery) OnlyIDX(ctx context.Context) uint32 {
...
func (tq *TeacherQuery) IDs(ctx context.Context) (ids []uint32, err error) {
...
func (tq *TeacherQuery) IDsX(ctx context.Context) []uint32 {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *TeacherQuery) Clone() *TeacherQuery {
	...
	return &TeacherQuery{
		...
		withCollege: tq.withCollege.Clone(),
...
func (tq *TeacherQuery) WithCollege(opts ...func(*CollegeQuery)) *TeacherQuery {
	query := (&CollegeClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withCollege = query
	return tq
}
...
func (tq *TeacherQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Teacher, error) {
	...
	if query := tq.withCollege; query != nil {
		if err := tq.loadCollege(ctx, query, nodes, nil,
			func(n *Teacher, e *College) { n.Edges.College = e }); err != nil {
			return nil, err
		}
	}
...
func (tq *TeacherQuery) loadCollege(ctx context.Context, query *CollegeQuery, nodes []*Teacher, init func(*Teacher), assign func(*Teacher, *College)) error {
	ids := make([]uint8, 0, len(nodes))
	nodeids := make(map[uint8][]*Teacher)
	for i := range nodes {
		fk := nodes[i].Cid
...
func (tq *TeacherQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Teacher, init func(*Teacher), assign func(*Teacher, *User)) error {
	...
	nodeids := make(map[uint32][]*Teacher)
	for i := range nodes {
		fk := nodes[i].UID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
...
func (tq *TeacherQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(teacher.Table, teacher.Columns, sqlgraph.NewFieldSpec(teacher.FieldID, field.TypeUint32))
	_spec.From = tq.sql
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withCollege != nil {
			_spec.Node.AddColumnOnce(teacher.FieldCid)
		}
		if tq.withUser != nil {
			_spec.Node.AddColumnOnce(teacher.FieldUID)
		}
	}
