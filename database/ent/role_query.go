...
func (rq *RoleQuery) FirstID(ctx context.Context) (id uint8, err error) {
	var ids []uint8
...
func (rq *RoleQuery) FirstIDX(ctx context.Context) uint8 {
...
func (rq *RoleQuery) OnlyID(ctx context.Context) (id uint8, err error) {
	var ids []uint8
...
func (rq *RoleQuery) OnlyIDX(ctx context.Context) uint8 {
...
func (rq *RoleQuery) IDs(ctx context.Context) (ids []uint8, err error) {
...
func (rq *RoleQuery) IDsX(ctx context.Context) []uint8 {
	ids, err := rq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (rq *RoleQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Role, init func(*Role), assign func(*Role, *Permission)) error {
	...
	byID := make(map[uint8]*Role)
	nids := make(map[uint16]map[*Role]struct{})
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				outValue := uint8(values[0].(*sql.NullInt64).Int64)
				inValue := uint16(values[1].(*sql.NullInt64).Int64)
				if nids[inValue] == nil {
...
func (rq *RoleQuery) loadRolePermission(ctx context.Context, query *RolePermissionQuery, nodes []*Role, init func(*Role), assign func(*Role, *RolePermission)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(rolepermission.FieldRid)
	}
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "rid" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
...
func (rq *RoleQuery) loadUserRole(ctx context.Context, query *UserRoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *UserRole)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userrole.FieldRid)
	}
...
func (rq *RoleQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(role.Table, role.Columns, sqlgraph.NewFieldSpec(role.FieldID, field.TypeUint8))
	_spec.From = rq.sql
