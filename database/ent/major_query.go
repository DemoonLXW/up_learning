...
// QueryCollege chains the current query on the "college" edge.
func (mq *MajorQuery) QueryCollege() *CollegeQuery {
	query := (&CollegeClient{config: mq.config}).Query()
...
func (mq *MajorQuery) QueryClasses() *ClassQuery {
...
func (mq *MajorQuery) FirstID(ctx context.Context) (id uint16, err error) {
	var ids []uint16
...
func (mq *MajorQuery) FirstIDX(ctx context.Context) uint16 {
...
func (mq *MajorQuery) OnlyID(ctx context.Context) (id uint16, err error) {
	var ids []uint16
...
func (mq *MajorQuery) OnlyIDX(ctx context.Context) uint16 {
...
func (mq *MajorQuery) IDs(ctx context.Context) (ids []uint16, err error) {
...
func (mq *MajorQuery) IDsX(ctx context.Context) []uint16 {
	ids, err := mq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (mq *MajorQuery) Clone() *MajorQuery {
	...
	return &MajorQuery{
		...
		withCollege: mq.withCollege.Clone(),
		withClasses: mq.withClasses.Clone(),
...
// WithCollege tells the query-builder to eager-load the nodes that are connected to
// the "college" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MajorQuery) WithCollege(opts ...func(*CollegeQuery)) *MajorQuery {
	query := (&CollegeClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withCollege = query
	return mq
}
...
func (mq *MajorQuery) WithClasses(opts ...func(*ClassQuery)) *MajorQuery {
	query := (&ClassClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withClasses = query
	return mq
}
...
func (mq *MajorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Major, error) {
	...
	if query := mq.withCollege; query != nil {
		if err := mq.loadCollege(ctx, query, nodes, nil,
			func(n *Major, e *College) { n.Edges.College = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withClasses; query != nil {
		if err := mq.loadClasses(ctx, query, nodes,
			func(n *Major) { n.Edges.Classes = []*Class{} },
			func(n *Major, e *Class) { n.Edges.Classes = append(n.Edges.Classes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MajorQuery) loadCollege(ctx context.Context, query *CollegeQuery, nodes []*Major, init func(*Major), assign func(*Major, *College)) error {
	ids := make([]uint8, 0, len(nodes))
	nodeids := make(map[uint8][]*Major)
	for i := range nodes {
		fk := nodes[i].Cid
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "cid" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MajorQuery) loadClasses(ctx context.Context, query *ClassQuery, nodes []*Major, init func(*Major), assign func(*Major, *Class)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint16]*Major)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(class.FieldMid)
	}
	query.Where(predicate.Class(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(major.ClassesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "mid" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (mq *MajorQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(major.Table, major.Columns, sqlgraph.NewFieldSpec(major.FieldID, field.TypeUint16))
	_spec.From = mq.sql
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withCollege != nil {
			_spec.Node.AddColumnOnce(major.FieldCid)
		}
	}
