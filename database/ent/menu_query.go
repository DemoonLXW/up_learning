...
func (mq *MenuQuery) QueryRole() *RoleQuery {
...
func (mq *MenuQuery) FirstID(ctx context.Context) (id uint8, err error) {
	var ids []uint8
...
func (mq *MenuQuery) FirstIDX(ctx context.Context) uint8 {
...
func (mq *MenuQuery) OnlyID(ctx context.Context) (id uint8, err error) {
	var ids []uint8
...
func (mq *MenuQuery) OnlyIDX(ctx context.Context) uint8 {
...
func (mq *MenuQuery) IDs(ctx context.Context) (ids []uint8, err error) {
...
func (mq *MenuQuery) IDsX(ctx context.Context) []uint8 {
	ids, err := mq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (mq *MenuQuery) WithRole(opts ...func(*RoleQuery)) *MenuQuery {
	query := (&RoleClient{config: mq.config}).Query()
...
func (mq *MenuQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Menu, error) {
	...
	if query := mq.withRole; query != nil {
		if err := mq.loadRole(ctx, query, nodes, nil,
			func(n *Menu, e *Role) { n.Edges.Role = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MenuQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *Role)) error {
	ids := make([]uint8, 0, len(nodes))
	nodeids := make(map[uint8][]*Menu)
	for i := range nodes {
		fk := nodes[i].Rid
...
func (mq *MenuQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(menu.Table, menu.Columns, sqlgraph.NewFieldSpec(menu.FieldID, field.TypeUint8))
	_spec.From = mq.sql
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withRole != nil {
			_spec.Node.AddColumnOnce(menu.FieldRid)
		}
	}
