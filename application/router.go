...
func SetupRouter(app *gin.Engine, controllers *controller.Controllers) *gin.Engine {
	...
	auth.Use(Auth(controllers.User.Services.User))
	{
		...
		role := auth.Group("/role")
		{
			...
			role.GET("/:id/get/permissions", Check(userService, []string{entity.RetrieveRole, entity.RetrievePermission}), controllers.Management.GetPermissionsByRoleId)
			role.POST("/modify/permissions", Check(userService, []string{entity.ModifyRole, entity.RetrievePermission}), controllers.Management.ModifyPermissionsForRoles)
		}
